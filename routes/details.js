...
router.get("/buy", (req, res) => {
    ...
    console.log("uname="+uname);
        var sql="select * from buy where product_id= ? and uname = ?"
        pool.query(sql, [lid,uname], (err, result) => {
            ...
            if(result.length>0){
                ...
                var sql = "INSERT INTO buy VALUES(?,?)";
...
router.get("/content", (req, res) => {
    ...
    (async function () {
        var sql = " select product_id from buy where uname=?";
        await new Promise(function (open) {
